...
const Journal = React.forwardRef<JournalHandle, Props>(function Journal(
  ...
  const emptyForm = () => ({ title: '', description: '', vividness: 1, tags: [] as string[] });
  const [form, setForm] = useState(emptyForm());

  const listOpacity = useRef(new Animated.Value(1)).current;
  const listTransY = useRef(new Animated.Value(0)).current;
  const panelOpacity = useRef(new Animated.Value(0)).current;
  const panelTransX = useRef(new Animated.Value(width)).current;
  ...
  useImperativeHandle(ref, () => ({
    openForm: () => {
      ...
      setEditing(false);
      setSelected(null);
      // Instantly switch to form — no animation, no flash
      listOpacity.setValue(0);
      listTransY.setValue(20);
      panelOpacity.setValue(1);
      panelTransX.setValue(0);
      setView('form');
    },
  }));
  ...
  // When dreams is cleared (data deleted), reset to list view
  useEffect(() => {
    if (dreams.length === 0) {
      setSelected(null);
      setEditing(false);
  ...
  const showPanel = (target: 'form' | 'detail', instant = false) => {
    ...
    Animated.parallel([
      ...
      Animated.timing(listTransY, { toValue: 20, duration: 220, useNativeDriver: true }),
      Animated.timing(panelOpacity, { toValue: 1, duration: 260, useNativeDriver: true }),
      Animated.timing(panelTransX, { toValue: 0, duration: 260, useNativeDriver: true }),
    ]).start();
  };
  ...
  const saveDream = async () => {
    ...
    // When editing, preserve the original dream's date so streak isn't affected
    const dreamDate = editing && selected
      ? selected.date
      : today.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    const dreamDateISO = editing && selected ? selected.dateISO : toLocalISO(today);

    const dream: Dream = {
      id: editing && selected ? selected.id : Date.now().toString(),
      title: form.title,
  ...
  const VividDots = ({ v }: { v: number }) => (
    <View style={styles.vividRow}>
      {[1,2,3,4,5].map(n => (
        <View key={n} style={[styles.vividDot, n <= v && styles.vividDotOn]} />
      ))}
    </View>
  );
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.listContent}>
          {dreams.length === 0 && (
            <Text style={styles.emptyText}>No dreams logged yet. Tap + to add one.</Text>
          )}
          {dreams.map(dream => (
            dream.noMemory ? (
              <TouchableOpacity
              ...
              <TouchableOpacity key={dream.id} style={styles.card} onPress={() => openDetail(dream)} activeOpacity={0.75}>
                <View style={styles.cardTop}>
                  <Text style={styles.cardTitle} numberOfLines={1}>{dream.title}</Text>
      ...
      <Animated.View
        style={[
          StyleSheet.absoluteFill,
          { opacity: panelOpacity, transform: [{ translateX: panelTransX }] },
          view === 'list' && { pointerEvents: 'none' },
        ]}
      >
          ...
          <View style={styles.root}>
            ...
            <ScrollView contentContainerStyle={styles.detailContent}>
              ...
              {!selected.noMemory && selected.vividness > 0 && (
                <View style={[styles.vividRow, { marginBottom: 20 }]}>
                  {[1,2,3,4,5].map(n => (
                    <View key={n} style={[styles.vividDotLg, n <= selected.vividness && styles.vividDotLgOn]} />
                  ))}
                </View>
              )}
        ...
        {(view === 'form' || (view === 'detail' && editing)) && (
          <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <View style={styles.header}>
              <TouchableOpacity onPress={() => editing ? setEditing(false) : showList()}>
                <Ionicons name="arrow-back" size={24} color={colors.textPrimary} />
              </TouchableOpacity>
              <Text style={styles.headerTitle}>{editing ? 'Edit Dream' : 'New Dream Entry'}</Text>
              <View style={{ width: 24 }} />
            </View>
            ...
            <ScrollView contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
              {alreadyLoggedToday && !editing && (
                <View style={styles.warnBanner}>
                  ...
                  <Text style={styles.warnText}>You already logged a dream today. You can still add another.</Text>
                </View>
              )}
              ...
              <Text style={[styles.wordCounter, overLimit && styles.wordCounterOver]}>
                {wordsLeft < 0 ? `${Math.abs(wordsLeft)} words over limit` : `${wordsLeft} words remaining`}
              </Text>

              <Text style={[styles.label, { marginTop: 20 }]}>VIVIDNESS</Text>
              <Animated.View
                style={[
                  styles.vividSelector,
                  { transform: [{ translateX: vividShake }] },
                ]}
              >
                {[1,2,3,4,5].map(n => (
                  <TouchableOpacity
                  ...
                  >
                    <View style={[styles.vividBtnInner, form.vividness >= n && styles.vividBtnInnerOn]} />
                  </TouchableOpacity>
                ))}
              </Animated.View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 20, fontWeight: '600', fontFamily: 'Nunito_600SemiBold', color: colors.textPrimary, flex: 1, textAlign: 'center' },
  addBtn: {
  ...
  warnText: {
    flex: 1, fontSize: 13, fontFamily: 'Nunito_300Light',
    color: '#fbbf24', lineHeight: 18,
  },

  listContent: { paddingHorizontal: 20, paddingBottom: 120 },
  emptyText: { color: colors.textMuted, fontFamily: 'Nunito_300Light', textAlign: 'center', marginTop: 60, fontSize: 15 },
  ...
  cardTitle: { fontSize: 17, fontWeight: '600', fontFamily: 'Nunito_600SemiBold', color: colors.textPrimary, flex: 1, marginRight: 8 },
  ...
  cardNoMemory: {
    ...
    borderRadius: 10, marginBottom: 8, paddingHorizontal: 14, paddingVertical: 10,
  },
  cardNoMemoryDate: { fontSize: 13, fontFamily: 'Nunito_300Light', color: colors.textMuted },
  cardNoMemoryLabel: { fontSize: 13, fontFamily: 'Nunito_300Light', color: 'rgba(255,255,255,0.2)', fontStyle: 'italic' },
  ...
  tag: { backgroundColor: 'rgba(139,92,246,0.2)', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 12 },
  ...
  detailDate: { fontSize: 14, fontFamily: 'Nunito_300Light', color: colors.textMuted, marginBottom: 16 },
  detailBody: { fontSize: 16, fontFamily: 'Nunito_300Light', color: colors.textPrimary, lineHeight: 26 },
  detailBodyMuted: { fontSize: 15, fontFamily: 'Nunito_300Light', color: colors.textMuted, fontStyle: 'italic' },

  formContent: { paddingHorizontal: 20, paddingBottom: 60 },
  label: {
    fontSize: 11, color: 'rgba(255,255,255,0.4)',
    letterSpacing: 1, marginBottom: 8, fontFamily: 'Nunito_600SemiBold',
  },
  input: {
    backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 12,
    borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.15)',
    padding: 14, color: colors.textPrimary, fontFamily: 'Nunito_300Light', fontSize: 15,
  },
  inputArea: { minHeight: 130, textAlignVertical: 'top' },
  wordCounter: { fontSize: 12, color: 'rgba(255,255,255,0.3)', textAlign: 'right', marginTop: 6, fontFamily: 'Nunito_300Light' },
  wordCounterOver: { color: '#f87171' },

  vividSelector: { flexDirection: 'row', gap: 14 },
  vividBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.08)', justifyContent: 'center', alignItems: 'center' },
  vividBtnOn: { backgroundColor: 'rgba(91,79,212,0.35)' },
  vividBtnInner: { width: 14, height: 14, borderRadius: 7, backgroundColor: 'rgba(255,255,255,0.25)' },
  vividBtnInnerOn: { backgroundColor: colors.primaryPurple },

  tagToggle: { backgroundColor: 'rgba(255,255,255,0.08)', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 16, marginBottom: 8 },
  ...
  saveBtn: { backgroundColor: colors.primaryPurple, borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 28 },
